...
inline size_t precedence(const char op) {
  ...
  throw std::runtime_error{
      string{"invalid operator in precedence() function: "} + op};
}
...
int main() {
  ...
  try {
    while (true) {
      std::cout << "Enter an arithmetic expression and press Enter - enter an "
                   "empty line to end:"
                << std::endl;
      ...
      if (lab) std::cout << 1 << "   " << operands.top() << std::endl;
      operands.push(
          std::stod(exp, &index));  // Push the first (lhs) operand on the stack
      ...
      std::cout << 1 << "   " << operands.top() << std::endl;
      if (lab) {
        operands.pop();
        std::cout << 1 << "   " << operands.top() << std::endl;
      }
